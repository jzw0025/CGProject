...
class DynamicObjects(object):
    ...
    def __init__(self, points, elements, normals=None, color=None, center=None):
        ...
        if center is None:
            ...
            if type(center) is not tuple:
                raise TypeError("the input center data must be a tuple")
            else:
                self.center = center
        ...
        if color is None:
            ...
            if type(color) is not tuple:
                raise TypeError("the input color data must be a tuple")
            else:
                self.color = color
        ...
        self.clipped = False # keep track of clipping state 
        self.global_state = True # default set the object to be global object, if there is crop, it changes the object into a local object
    ...
    def applyClipPlane(self, clipPlaneObject):
        ...
        if self.clipped: # each global object can only be cropped once
            return
        ...
        clip_logic = np.logical_and(np.logical_and(x_axis, y_axis), z_axis)   
        ...
        self.elements = new_elements # only the element array needed to be updated
        ...
        ## update the center (used for tracking)
        self.center = (self.points[0,:].mean(), self.points[1,:].mean(), self.points[2,:].mean())
        
        self.clipped = True # update clipped history for this object
        
        self.global_state = False # change this into a local object
