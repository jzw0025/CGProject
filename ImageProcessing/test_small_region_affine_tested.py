...
import numpy as np
import array
mlab.figure(bgcolor=(0.5,0.5,0.5), size=(400, 400))

#src = mlab.pipeline.scalar_field(ndimage.gaussian_filter(orig_volume1,3))
#src2 = mlab.pipeline.scalar_field(ndimage.gaussian_filter(orig_volume2,3))
## Our data is not equally spaced in all directions:
#src.spacing = [1, 1, 1]
#src2.spacing = [1, 1, 1]
#src.update_image_data = True
#src2.update_image_data = True
#mlab.pipeline.iso_surface(src, contours=[70], colormap='Spectral')
#mlab.pipeline.iso_surface(src2, contours=[70], colormap='Oranges')
#
#thr = mlab.pipeline.threshold(src, low=70)
...
#                                vmax=0.9)                       
#                                
#cut_plane.implicit_plane.origin # extact the point in the view
#
import scipy.io as sio

volume000 = sio.loadmat('/Users/junchaowei/Desktop/SpaceRegistration_000_125/volume000.mat')     
...
#zpos[0] = z
#zpos[1] = 0
#color = np.ones(xpos.shape)

#### help function #####
def reshapePoint(point1, point2):
    xpos = np.ndarray((2)) 
    ypos = np.ndarray((2)) 
    zpos = np.ndarray((2)) 
    xpos[0] = point1[0]
